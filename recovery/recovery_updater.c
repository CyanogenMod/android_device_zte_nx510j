...
Value * VerifyBasebandFn(const char *name, State *state, int argc, Expr *argv[]) {
    ...
    for (i = 0; i < argc; i++) {
        ...
        if (strncmp(baseband_version + BASEBAND_VER_STR_START_LEN, current_baseband_version + BASEBAND_VER_STR_START_LEN, strlen(baseband_version) - BASEBAND_VER_STR_START_LEN) == 0) {
